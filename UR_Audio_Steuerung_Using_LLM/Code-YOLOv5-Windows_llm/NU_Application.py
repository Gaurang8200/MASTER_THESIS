...
def move_to_object(robot_ip):
    ...
    with open(r'txt_file/robot_coordinates.txt', 'r') as file:
...
def pick_the_object(robot_ip):
    file_path = r'txt_file/robot_RPY.txt'
...
def detect_object():
    detection_command = [sys.executable, 'UR_detection.py']
    subprocess.run(detection_command, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

def convert_pixel_to_robot():
    detection_command = [sys.executable, 'UR_pixel2robot.py']
    subprocess.run(detection_command, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

def pca_calculation():
    detection_command = [sys.executable, 'UR_pca.py']
    subprocess.run(detection_command, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

def direction_object():
    detection_command = [sys.executable, 'UR_direction.py']
    subprocess.run(detection_command, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
while True:
    ...
    file_path = r'txt_file/label.txt'
    with open(file_path, 'r') as file:
