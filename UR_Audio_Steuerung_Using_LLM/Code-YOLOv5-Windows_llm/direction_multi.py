# MO_Changes
import cv2
import numpy as np
import math
from scipy.spatial.transform import Rotation as R
import os
from pathlib import Path

def draw_arrows(image, origin, length1, angle1, length2, angle2):
    
    # Convert angles from degrees to radians
    angle1_rad = np.deg2rad(angle1)
    angle2_rad = np.deg2rad(angle2)
    
    # Step 1: Convert RPY angles from degrees to radians
    rpy_degrees = [180, 0, angle1]
    rpy_radians = np.deg2rad(rpy_degrees)

    # Step 2: Convert RPY (roll, pitch, yaw) to a rotation matrix
    rotation_matrix = R.from_euler('xyz', rpy_radians).as_matrix()

    # Step 3: Convert the rotation matrix to a rotational vector (axis-angle representation)
    rot_vec = R.from_matrix(rotation_matrix).as_rotvec()

    # Step 4: Round the rotational vector to four decimal places
    rot_vec_rounded = np.round(rot_vec, 3)
    print(rot_vec_rounded)

    # Calculate the end points for each arrow
    end1 = (int(origin[0] + length1 * np.cos(angle1_rad)), int(origin[1] - length1 * np.sin(angle1_rad)))
    end2 = (int(origin[0] + length2 * np.cos(angle2_rad)), int(origin[1] - length2 * np.sin(angle2_rad)))

    origin_for_robot = (int((origin[0]*2560)/1920), int((origin[1]*1472)/1080))
    
    with open("txt_file/direction.txt", "w") as file:
        file.write(f"Origin for Robot: {origin_for_robot}\n")
        #file.write(f"Origin: {origin}\n")
        #file.write(f"Ends 1: {end1}\n")
        #file.write(f"Ends 2: {end2}\n")
        #file.write(f"Length 1: {length1}\n")
        #file.write(f"Length 2: {length2}\n")    
        file.write(f"Angle 1: {angle1}\n")
        file.write(f"Angle 2: {angle2}\n")
    
    with open("txt_file/robot_RPY.txt", "w") as file:
        file.write(f"{rot_vec_rounded}")

    # Convert origin to integer
    origin = (int(origin[0]), int(origin[1]))

    # Draw the arrows
    cv2.arrowedLine(image, origin, end1, (0, 0, 255), 3)  # Red arrow
    cv2.arrowedLine(image, origin, end2, (0, 255, 0), 3)  # Green arrow
    
    return image

# Load all Image Path
def image_path():
    with open(os.path.join("txt_file", "detect_img_path.txt"), "r") as file:
        stored_path = Path(file.read().strip())
    candidates = [stored_path]
    if not stored_path.is_absolute():
        candidates.extend((Path.cwd() / stored_path, Path.cwd() / "yolov5" / stored_path))
    for candidate in candidates:
        if candidate.is_file():
            return str(candidate.resolve())
    raise FileNotFoundError(f"Detection image does not exist: {stored_path}")

# Load an existing image
image_path = image_path()
image = cv2.imread(image_path)

# If the image is not found, raise an error
if image is None:
    raise FileNotFoundError("The specified image path does not exist or the image could not be loaded.")

# Calculate centre of detected Object
def label_path():
    """
    Lese Label-Pfad: Versuche zuerst final_object_label.txt, 
    dann Fallback zu label_path.txt
    """
    # Try final label file first (from precision detection filtering) 
    final_label_file = "txt_file/final_object_label.txt"
    if os.path.exists(final_label_file):
        print(f"DIRECTION: Using final label file: {final_label_file}")
        return final_label_file
    
    # Fallback to standard label_path.txt  
    standard_label_path_file = "txt_file/label_path.txt"
    if os.path.exists(standard_label_path_file):
        with open(standard_label_path_file, "r") as file:
            label_path_content = file.read().strip()
            if label_path_content and os.path.exists(label_path_content):
                print(f"DIRECTION: Using standard label path: {label_path_content}")
                return label_path_content
            else:
                print(f"DIRECTION WARNING: Standard label path does not exist: {label_path_content}")
    
    # Final fallback - return the final label file path even if it doesn't exist
    # (will be handled by calculate_origin)
    print(f"DIRECTION: Fallback to final label file: {final_label_file}")
    return final_label_file
    
# Function to read the file and calculate the origin
def calculate_origin(file_path):
    """
    Lese Origin-Koordinaten: Versuche zuerst final_object_center_point.txt,
    dann Fallback zu label-basierter Berechnung
    """
    try:
        # Try final center point file first (from precision detection filtering)
        final_center_file = "txt_file/final_object_center_point.txt"
        if os.path.exists(final_center_file):
            with open(final_center_file, 'r') as file:
                content = file.read().strip()
                if content:
                    coords = content.split()
                    if len(coords) >= 2:
                        x = float(coords[0])
                        y = float(coords[1])
                        origin = (x, y)
                        print(f"DIRECTION: Using final center point: {origin}")
                        return origin
        
        # Fallback to label-based calculation (existing logic)
        print(f"DIRECTION: Using label-based calculation from: {file_path}")
        
        # Open the file and read the content
        with open(file_path, 'r') as file:
            content = file.read().strip()
        
        # Split the content into a list of numbers
        numbers = list(map(int, content.split()))
        
        if len(numbers) != 5:
            raise ValueError("File does not contain exactly 5 numbers")

        # Calculate x and y
        x = (numbers[1] + numbers[3]) / 2
        y = (numbers[2] + numbers[4]) / 2

        # Create the origin tuple
        origin = (x, y)
        print(f"DIRECTION: Calculated origin from label: {origin}")
        
        return origin

    except Exception as e:
        print(f"DIRECTION ERROR: An error occurred: {e}")
        # Return a default origin as final fallback
        return (640, 480)  # Default center point

# Calculate the origin using the function
origin = calculate_origin(label_path())

def calculate_vectors(file_path):
    try:
        # Open the file and read the content
        with open(file_path, 'r') as file:
            lines = file.readlines()

        # Process each line to extract vectors
        vectors = []
        for line in lines:
            line = line.strip().strip('[]')
            if line:
                vector = line.split()
                if len(vector) == 2:
                    vectors.append(list(map(float, vector)))

        if len(vectors) != 2:
            raise ValueError("File does not contain exactly 2 vectors")

        # Calculate lengths and angles for both vectors
        x1, y1 = vectors[0]
        x2, y2 = vectors[1]

        length1 = math.sqrt(x1**2 + y1**2)
        angle1 = math.degrees(math.atan2(y1, x1))

        length2 = math.sqrt(x2**2 + y2**2)
        angle2 = math.degrees(math.atan2(y2, x2))

        return length1, angle1, length2, angle2

    except Exception as e:
        print(f"An error occurred while calculating vectors: {e}")
        return None, None, None, None

# Calculate lengths and angles using the function
path = os.path.join("txt_file", "vectors.txt")
length1, angle1, length2, angle2 = calculate_vectors(path)

# Draw the arrows on the image
image_with_arrows = draw_arrows(image, origin, length1, angle1, length2, angle2)

output_path_direction = str(Path(image_path).with_name("direction_1.jpg"))

# Save the image to a desired path
cv2.imwrite(output_path_direction, image_with_arrows)

print(f"Image with detected direction saved successfully at {output_path_direction}")
