...
def load_detection_data():
    ...
    try:
        json_path = os.path.join("txt_file", "detected_objects.json")
        if os.path.exists(json_path):
            with open(json_path, 'r') as f:
                data = json.load(f)
            print(f"SUCCESS: Loaded detection data: {data.get('count', 0)} objects")
            return data
        else:
...
def save_detection_results(results):
    ...
    try:
        json_path = os.path.join("txt_file", "detection_results.json")
        with open(json_path, 'w') as f:
            json.dump(results, f, indent=2)
        print(f"FILE: Detection results saved to {json_path}")
        return True
    except Exception as e:
        print(f"ERROR: Error saving detection results: {e}")
        return False
