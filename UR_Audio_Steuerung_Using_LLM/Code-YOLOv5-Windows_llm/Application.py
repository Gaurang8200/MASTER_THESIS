...
def detect_object():
    detection_command = [sys.executable, 'detection.py']
    subprocess.run(detection_command, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

def convert_pixel_to_robot():
    detection_command = [sys.executable, 'pixel2robot.py']
    subprocess.run(detection_command, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

def pca_calculation():
    detection_command = [sys.executable, 'pca.py']
    subprocess.run(detection_command, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

def direction_object():
    detection_command = [sys.executable, 'direction.py']
    subprocess.run(detection_command, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
