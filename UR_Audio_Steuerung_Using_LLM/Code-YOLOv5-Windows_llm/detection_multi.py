...
class MultiObjectDetector:
        ...
        try:
            ...
            if os.path.exists(json_path):
                ...
                if self.detected_objects:
                    ...
                    # **NEW: Write center point files for ALL detected objects**
                    self._write_center_points_for_all_objects()
                    
                    print(f"INFO: Center point files created for all {len(self.detected_objects)} detected objects")
                    ...
                    # **FIRST DETECTION: Only coordinates calculated, no PCA/Direction/Border**
                    print("FIRST DETECTION: Only coordinates calculated, no PCA/Direction/Border processing")
                    print("INFO: This is the first detection for object localization only")
                    print("INFO: PCA/Direction/Border analysis will be done in second detection script")
    ...
    def _write_center_points_for_all_objects(self):
        """Write center point files for all detected objects"""
        try:
            txt_dir = TXT_FILE_DIR
            for obj in self.detected_objects:
                ...
                if robot_center:
                    center_x, center_y = robot_center
                    center_point_path = os.path.join(txt_dir, f"center_point_object_{obj['id']}.txt")
                    with open(center_point_path, 'w') as f:
                        f.write(f"{center_x} {center_y}")
                    print(f"DEBUG: Wrote center_point_object_{obj['id']}.txt with robot coordinates")
                else:
                    print(f"WARNING: Failed to convert pixel center {pixel_center} to robot coordinates")
        except Exception as e:
            print(f"ERROR: Failed to write center point files: {e}")
            import traceback
            traceback.print_exc()
