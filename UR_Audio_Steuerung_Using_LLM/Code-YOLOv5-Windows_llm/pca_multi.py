...
def read_path():
    ...
    dann Fallback zu crop_img_path.txt
    """
    ...
    if os.path.exists(final_crop_file):
        with open(final_crop_file, "r") as file:
            ...
            if crop_path and os.path.exists(crop_path):
                print(f"PCA: Using final crop path: {crop_path}")
                return crop_path
            else:
                print(f"PCA WARNING: Final crop path does not exist: {crop_path}")
    ...
    if os.path.exists(standard_crop_file):
        with open(standard_crop_file, "r") as file:
            crop_path = file.read().strip()
            if crop_path and os.path.exists(crop_path):
                print(f"PCA: Using standard crop path: {crop_path}")
                return crop_path
            else:
                print(f"PCA WARNING: Standard crop path does not exist: {crop_path}")
    
    # If both files fail, raise an error
    raise FileNotFoundError("PCA ERROR: No valid crop image path found in final_object_crop_path.txt or crop_img_path.txt")
